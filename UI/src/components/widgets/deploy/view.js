(function () {
    ...
    function deployViewController($scope, DashStatus, deployData, DisplayState, $q, $uibModal) {
        ...
        if ($scope.widgetConfig.options.ignoreRegex !== undefined && $scope.widgetConfig.options.ignoreRegex !== null && $scope.widgetConfig.options.ignoreRegex !== '') {
		 ...
		 var deploymentJobs = ['App_Deploy_FAT','App_Deploy_NFR','App_Deploy_Staging'];
		 var count = 0;
		 //ctrl.load =  captureDeploymentStatus(deploymentJobs[count],count,deploymentJobs); ;
                ...
                var success = function(json) {
				 ...
				 // alert("succeesss");
				  ...
				  $.each(nonSuccessfulJobs, function( index, value ) {
					 ...
					 var prodCheck = jobName.includes("PROD");
					 
					   if(deployCheck == true && prodCheck == false){
							...
							var successCountId = jobName+"SuccessCnt";
							var failureCountId = jobName+"failureCnt";
							...
							var trSuccess = (
									...
									'<td width="25%" style="color: green;" align="center" id="'+successCountId +'">'  + countSuccess + '<span class="fa fa-arrow-up"></span></td>'+
									'<td width="25%" style="color: red;" align="center"  id="'+failureCountId  +'">'   + countFailure + '<span class="fa fa-arrow-down"></span></td>' +
								     +'</tr>'
								);
				       ...
				       */
					    captureDeploymentStatus(deploymentJobs[count],count,deploymentJobs); 
				};
		...
		function captureDeploymentStatus(jobName,count,deploymentJobs)
		{
			...
			//**************************************************
						  ...
						  $.each(successfulJobs, function( index, value ) {
							...
							// alert("successfulJobs:::"+countSuccess +":::"+ countFailure);
						       ...
						       document.getElementById(jobName+"SuccessCnt").innerHTML = countSuccess + '<span class="fa fa-arrow-up"></span>' ;
							   document.getElementById(jobName+"failureCnt").innerHTML = countFailure + '<span class="fa fa-arrow-down"></span>' ;
							   count = count+1 ;
							  
							   if(count <  deploymentJobs.length){
								//   alert("hehe==");
